...
function verify(token: string, b64Secrets: string[]): boolean {
	...
	const payloadString = base64.decode(B64Payload);
	...
	if (!isValidInteger(header.kid, 0, secretsLen, true)) return false;
	...
	const expectedSignature = createHmac("sha256", secret)
		.update(`${B64Header}.${B64Payload}`)
		.digest("base64");
	if (!safeCompare(expectedSignature, B64Signature)) return false;
